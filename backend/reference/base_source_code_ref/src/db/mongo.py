# -*- coding: utf-8 -*-
"""
MongoDB Connection — Motor (async) + Umongo (ODM)

Pattern: Single global MotorAsyncIOInstance.
- init_mongo()  : goi 1 lan trong lifespan startup
- close_mongo() : goi 1 lan trong lifespan shutdown
- mongo_instance: import o moi model file de @register
...
# Moi model dang ky chung instance nay: @mongo_instance.register
...
def get_motor_client() -> AsyncIOMotorClient:
    ...
    if _motor_client is None:
        raise RuntimeError("MongoDB client is not initialized. Call init_mongo() first.")
    return _motor_client


def init_mongo() -> MotorAsyncIOInstance:
    """
    Initialize Async MongoDB client and bind it to Umongo instance.
    ...
    if _motor_client is None:
        ...
        logger.info(f"Motor & Umongo instance initialized for db: {settings.MONGO_DB}")
